...
CREATE TYPE design_request_status_enum AS ENUM (
  'draft',
  'submitted',
  'triaged',
  'queued',
...
CREATE TYPE creative_task_status_enum AS ENUM (
  'pending',
  'ready',
  'in_progress',
  'blocked',
...
CREATE TYPE feedback_target_enum AS ENUM (
  'design_request',
  'creative_task',
  'deliverable',
  'asset_version'
...
ADD CONSTRAINT fk_deliverables_final_version
FOREIGN KEY (final_version_id) REFERENCES asset_versions(id);
...
CREATE INDEX idx_design_requests_status_priority_sla
  ON design_requests (status, priority, sla_due_at);

CREATE INDEX idx_design_requests_project_brand_campaign
  ON design_requests (project_id, brand_id, campaign_id);

CREATE INDEX idx_creative_tasks_request_status_due
  ON creative_tasks (design_request_id, status, due_at);

CREATE INDEX idx_deliverables_request_status
  ON deliverables (design_request_id, status);

CREATE INDEX idx_approval_steps_status_due
  ON approval_steps (status, due_at);

CREATE INDEX idx_asset_versions_asset_file_version
  ON asset_versions (asset_file_id, version_number DESC);

CREATE INDEX idx_templates_brand_type_status
  ON templates (brand_id, template_type, status);

CREATE INDEX idx_asset_files_brand_campaign
  ON asset_files (brand_id, campaign_id);
...
CREATE TRIGGER trg_creative_tasks_updated_at
...
CREATE TRIGGER trg_asset_files_updated_at
